...
- Performance improvements for nodes to support higher transaction volumes, especially for low powered devices like RaspBerry Pi. Full details at [#9050](https://github.com/HDDcoin-Network/hddcoin-blockchain/pull/9050).
...
- Integrated the Bladebit plotter to CLI and GUI. Thanks @harold-b for all your hard work on this, and welcome again to the HDDcoin Network team!
- Added the Madmax plotter to CLI and GUI. Thanks @madMAx43v3r for your support!
- Added option to configure your node to testnet using to `hddcoin init --testnet`.
...
- PlotNFT transactions via CLI (e.g. `hddcoin plotnft join`) now accept a fee parameter, but it is not yet operable.

## 1.2.10 HDDcoin blockchain 2021-10-25

We have some great improvements in this release: We launched our migration of keys to a common encrypted keyring.yaml file, and we secure this with an optional passphrase in both GUI and CLI. We've added a passphrase hint in case you forget your passphrase. More info on our [wiki](https://github.com/HDDcoin-Network/hddcoin-blockchain/wiki/Passphrase-Protected-HDDcoin-Keys-and-Key-Storage-Migration). We also launched a new Chialisp compiler in clvm_tools_rs which substantially improves compile time for Chialisp developers. We also addressed a widely reported issue in which a system failure, such as a power outage, would require some farmers to sync their full node from zero. This release also includes several other improvements and fixes.

### Added

- Added support for keyring migration from keychain, and the addition of passphrase support. Learn more at our [wiki](https://github.com/HDDcoin-Network/hddcoin-blockchain/wiki/Passphrase-Protected-HDDcoin-Keys-and-Key-Storage-Migration).
- Enabled experimental use of a new Chialisp compiler in clvm_tools_rs in hddcoin-blockchain, which is off by default, and substantially improves compile time.
...
- Thanks @cross for change that allows using IPv6 address in config.yaml for remote harvesters and other hddcoin services.
...
- Updated the old comment on hddcoin/util/streamable.py with newer developer documentation.
...
- Thanks @jcteng for fixing a bug on the HDDcoin DID wallet that showed 'byte:'' instead of 'byte'.
...
- Beta builds are built on every merge to main, and are now available from <https://hddcoin.org/download/>.
...
- Switched IP address lookup to first use HDDcoin's service ip.hddcoin.org.
...
- Thanks @mishan for your change to 'hddcoin plotnft show' to display Percent Successful Points.
...
- Thanks to @yshklarov for help fixing [#7273](https://github.com/HDDcoin-Network/hddcoin-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.
...
- RC3 is a new chain to support the last major chialisp changes. THDD from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- 'hddcoin wallet send' now takes in THDD or HDD as units instead of bytes.
