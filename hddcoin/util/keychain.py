...
DEFAULT_USER = f"user-hddcoin-{CURRENT_KEY_VERSION}"  # e.g. user-hddcoin-1.8
DEFAULT_SERVICE = f"hddcoin-{DEFAULT_USER}"  # e.g. hddcoin-user-hddcoin-1.8
...
def supports_keyring_passphrase() -> bool:
    # Support can be disabled by setting HDDCOIN_PASSPHRASE_SUPPORT to 0/false
    return os.environ.get("HDDCOIN_PASSPHRASE_SUPPORT", "true").lower() in ["1", "true"]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from hddcoin.cmds.passphrase_funcs import prompt_for_passphrase
