...
from hddcoin.cmds.init_funcs import check_keys, hddcoin_init
from hddcoin.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from hddcoin.daemon.keychain_server import KeychainServer, keychain_commands
from hddcoin.daemon.windows_signal import kill
from hddcoin.plotters.plotters import get_available_plotters
...
service_plotter = "hddcoin_plotter"
...
class WebSocketServer:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["hddcoin", "plotters", plotter]
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    if service_command == "hddcoin_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
