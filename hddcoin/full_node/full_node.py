...
from hddcoin.consensus.pot_iterations import calculate_sp_iters
...
from hddcoin.full_node.full_node_store import FullNodeStore, FullNodeStorePeakResult
...
from hddcoin.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from hddcoin.protocols.full_node_protocol import (
    RequestBlocks,
    RespondBlock,
    RespondBlocks,
    RespondSignagePoint,
...
from hddcoin.protocols.wallet_protocol import CoinState, CoinStateUpdate
from hddcoin.server.node_discovery import FullNodePeers
...
from hddcoin.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
...
from hddcoin.types.transaction_queue_entry import TransactionQueueEntry
...
from hddcoin.util.errors import ConsensusError, Err, ValidationError
from hddcoin.util.ints import uint8, uint32, uint64, uint128
from hddcoin.util.path import mkdir, path_from_root
from hddcoin.util.safe_cancel_task import cancel_task_safe
    ...
    async def respond_unfinished_block(
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'hdd')}, "
                f"validation time: {validation_time:0.4f} seconds, {pre_validation_log}"
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
