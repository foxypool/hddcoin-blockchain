...
from hddcoin.protocols.protocol_state_machine import message_requires_reply
from hddcoin.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS, API_EXCEPTION_BAN_SECONDS
from hddcoin.protocols.shared_protocol import protocol_version
from hddcoin.server.introducer_peers import IntroducerPeers
from hddcoin.server.outbound_message import Message, NodeType
from hddcoin.server.ssl_context import private_ssl_paths, public_ssl_paths
...
class HDDcoinServer:
    def __init__(
        ...
        hddcoin_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        ...
        self.hddcoin_ca_crt_path, self.hddcoin_ca_key_path = hddcoin_ca_crt_key
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.hddcoin_ca_crt_path, self.hddcoin_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.hddcoin_ca_crt_path, self.hddcoin_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSHDDcoinConnection] = None
        try:
